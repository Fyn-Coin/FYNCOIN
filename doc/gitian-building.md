...
*Setup instructions for a gitian build of Fyn Coin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Fyn Coin
...
- [Building Fyn Coin](#building-fyncoin)
...
Clone the git repositories for fyncoin and gitian and then checkout the fyncoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the fyncoin repository
...
To build Fyn Coin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the fyncoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/fyncoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/fyncoin/fyncoin
...
For example:
```bash
URL=https://github.com/konez2k/fyncoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit fyncoin=${COMMIT} --url fyncoin=${URL} ../fyncoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit fyncoin=${COMMIT} --url fyncoin=${URL} ../fyncoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit fyncoin=${COMMIT} --url fyncoin=${URL} ../fyncoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/fyncoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/fyncoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/fyncoin-build.assert
```
...
[fyncoin/gitian.sigs](https://github.com/fyncoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
