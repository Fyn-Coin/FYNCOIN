...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a FYN address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no fyncoin: URI
...
{
    // Convert fyncoin:// to fyncoin:
    //
    //    Cannot handle this later, because fyncoin:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open fyncoin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "FynCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for FynCoin.lnk
...
{
    return GetAutostartDir() / "fyncoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a fyncoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=FynCoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the fyncoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add fyncoin app to startup item list
